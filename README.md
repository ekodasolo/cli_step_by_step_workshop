...
複数の参加者が同じ AWS アカウントでワークショップを行うと、リソース名や CIDR が衝突します。
参加者ごとに **プレフィックスと CIDR を変える**ことで、互いに影響なく同時進行できます。

### 方針

各参加者に番号（01, 02, ...）を割り当て、以下を変更します。

| 参加者 | プレフィックス | VPC CIDR | Subnet 1a | Subnet 1c |
|--------|---------------|----------|-----------|-----------|
| user01 | `cli-ws-01` | `10.1.0.0/16` | `10.1.1.0/24` | `10.1.2.0/24` |
| user02 | `cli-ws-02` | `10.2.0.0/16` | `10.2.1.0/24` | `10.2.2.0/24` |
| user03 | `cli-ws-03` | `10.3.0.0/16` | `10.3.1.0/24` | `10.3.2.0/24` |
| ... | `cli-ws-NN` | `10.NN.0.0/16` | `10.NN.1.0/24` | `10.NN.2.0/24` |

### 参加者が変更する箇所

各手順書の冒頭にある「処理パラメータの準備」セクションで、以下の変数を自分の番号に合わせて変更します。

```bash
# 例: 参加者 03 の場合
VPC_CIDR="10.3.0.0/16"
SUBNET_1A_CIDR="10.3.1.0/24"
SUBNET_1C_CIDR="10.3.2.0/24"
```

また、リソース名の `cli-workshop` 部分を自分のプレフィックスに置き換えます。

```bash
# 例: 参加者 03 の場合
VPC_NAME="cli-ws-03-vpc"
SUBNET_NAME="cli-ws-03-public-1a"
# ... 以降のリソース名も同様
```

### 講師用スクリプトでの対応

`scripts/workshop.env` を参加者番号に合わせて書き換えるだけで対応できます。

```bash
# scripts/workshop.env — 参加者 03 用に変更
PROJECT_NAME="cli-ws-03"
VPC_CIDR="10.3.0.0/16"
SUBNET_1A_CIDR="10.3.1.0/24"
SUBNET_1C_CIDR="10.3.2.0/24"
```
...
- **IAM ロール名はグローバル**（リージョンではなくアカウント単位で一意）なので、プレフィックスを変えることで衝突を回避できます
- **ALB 名は 32 文字以内**の制限があります。プレフィックスを短くしているのはこのためです
- クリーンアップ時は、自分のプレフィックスのリソースだけを削除するよう注意してください
